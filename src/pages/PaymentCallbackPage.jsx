...
export function PaymentCallbackPage() {
  ...
  useEffect(() => {
    ...
    async function verify() {
      try {
        ...
        if (active) {
          setState({ loading: false, error: error.message || 'Payment verification failed', order: null });
        }
      }
    }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4 py-12">
      <div className="w-full max-w-2xl rounded-[2rem] border border-[var(--line)] bg-[#0c0d0d] p-8 shadow-[0_24px_90px_rgba(0,0,0,.48)]">
        {state.loading ? (
          <div className="text-center">
            ...
            <p className="mt-3 text-sm leading-7 text-[var(--text-secondary)]">Please wait while we confirm your Flutterwave transaction.</p>
          </div>
        ) : state.error ? (
          <div className="text-center">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          <Link to="/" className="rounded-full bg-[var(--gold)] px-5 py-3 text-sm font-semibold text-[#111]">Back to storefront</Link>
          <Link to="/admin" className="rounded-full border border-[var(--line)] px-5 py-3 text-sm text-[var(--text-primary)]">Open admin panel</Link>
        </div>
      </div>
    </div>
  );
